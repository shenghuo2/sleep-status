...
2. API 接口说明：
   ...
   - 获取睡眠统计数据：
     ...
     - `show_sleep`：可选，是否显示当前睡眠状态，1表示显示，0表示不显示，默认为0
     ...
     - `sleep`：当前睡眠状态，仅当 `show_sleep=1` 时才显示
     - `current_sleep_at`：当前睡眠开始时间（Unix 时间戳，秒），仅当当前状态为睡眠时才显示
...
- 新功能
  ...
  - 支持在 `/sleep-stats` 中显示当前睡眠状态（可选）
  - 当处于睡眠状态时，自动显示当前睡眠开始时间
- 改进
  - 支持短睡眠时间段（小于3小时）的处理
  - 当数据不足时，自动调整实际统计的天数
  - 可选是否显示原始时间字符串，默认不显示
  - 可选是否显示当前睡眠状态，通过 `show_sleep=1` 参数控制
- 代码优化
